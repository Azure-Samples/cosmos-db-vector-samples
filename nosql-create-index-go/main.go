...
func main() {
	...
	for _, containerName := range cfg.ContainerNames {
		...
		count, err := CountDocuments(ctx, containerClient, cfg.PartitionKeyFieldValue)
		if err != nil {
			log.Printf("  ⚠ Could not verify document count: %v", err)
		} else {
			fmt.Printf("  ✓ Verified: %d documents in partition '%s'\n", count, cfg.PartitionKeyFieldValue)
		}
	}

	// Allow index to stabilize after insertions
	// QuantizedFlat indexes may need time to process all documents
	...
	// --- Phase 4: Cleanup — Delete containers (ARM SDK) ---
	fmt.Println("\n=== Phase 4: Cleanup ===")
	if err := DeleteContainers(ctx, credential, cfg); err != nil {
