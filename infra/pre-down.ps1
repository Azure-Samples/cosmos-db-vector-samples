...
DEPLOYMENT SCENARIOS:
  ...
  2. VECTOR-SEARCH: Full-stack deployment (database + containers + indexes pre-created)
     - Scenario triggered by: $AZURE_COSMOSDB_DATABASENAME environment variable set
...
$VectorSearchDatabaseName = $env:AZURE_COSMOSDB_DATABASENAME
$IsVectorSearchScenario = -not [string]::IsNullOrEmpty($VectorSearchDatabaseName)

if ($IsCreateIndexScenario -and -not $IsVectorSearchScenario) {
...
elseif ($IsVectorSearchScenario -and -not $IsCreateIndexScenario) {
    Write-Host "Detected VECTOR-SEARCH scenario — cleaning up vector-search samples"
    Write-Host "  (Environment variable AZURE_COSMOSDB_DATABASENAME = '$VectorSearchDatabaseName')"
    ...
    Write-Warning "Unable to determine the deployment scenario. Exactly one of AZURE_COSMOSDB_CREATE_INDEX_DATABASENAME or AZURE_COSMOSDB_DATABASENAME must be set."
    Write-Warning "Skipping sample data cleanup rather than assuming a scenario."
    exit 0
}
