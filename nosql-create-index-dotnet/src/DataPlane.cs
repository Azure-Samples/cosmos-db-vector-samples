...
public static partial class DataPlane
{
    ...
    public static async Task<IReadOnlyList<HotelDocument>> ReadDocumentsAsync(string dataFilePath, CancellationToken cancellationToken)
    {
        ...
        var documents = await System.Text.Json.JsonSerializer.DeserializeAsync<List<HotelDocument>>(stream, JsonOptions, cancellationToken)
        ...
        for (int idx = 0; idx < documents.Count; idx++)
        {
            ...
            if ((document.Embedding is null || document.Embedding.Length == 0) &&
                document.DescriptionVector is { Length: > 0 })
            {
                document.Embedding = document.DescriptionVector;
            }

            if (document.Embedding is null || document.Embedding.Length == 0)
            {
                throw new InvalidOperationException(
                    $"Document at index {idx} (HotelId={document.HotelId}) missing vector values in 'embedding'/'DescriptionVector'.");
            }
        }
    ...
    private static PartitionKey BuildRegionPartitionKey(string region)
    {
            ...
            while (iterator.HasMoreResults)
            {
                ...
                foreach (var item in response)
                {
                    await container.DeleteItemAsync<ContainerItemIdentityRow>(
                        item.Id,
                        BuildRegionPartitionKey(item.Region),
                        cancellationToken: cancellationToken);
                }
            }
        }
