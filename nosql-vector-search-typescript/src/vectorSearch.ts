...
type VectorAlgorithm = 'diskann' | 'quantizedflat' | 'flat';
...
const algorithmConfigs: Record<VectorAlgorithm, AlgorithmConfig> = {
    diskann: {
        ...
        description: 'Graph-based algorithm - optimized for low latency, highly scalable workloads with efficient RU consumption'
    },
    quantizedflat: {
        containerName: 'hotels_quantizedflat',
        algorithmName: 'QuantizedFlat',
        description: 'Vector quantization algorithm - efficient RU consumption at scale with good recall'
    },
    flat: {
        containerName: 'hotels_flat',
        algorithmName: 'Flat',
        description: 'Linear scan algorithm - best for testing and very small scenarios with small dimensional vectors'
    }
};
...
const config = {
    ...
    dbName: "Hotels",
    algorithm: (process.env.VECTOR_ALGORITHM || 'diskann').toLowerCase() as VectorAlgorithm,
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            console.log(`ℹ️  ${algorithmConfig.description}`);
